"""
Multi-Node Zero-Trust Authentication Layer (integrated with ntfy alerting)
------------------------------------------------------------------------------
Sends real-time push notifications via ntfy whenever a node is
quarantined or re-authenticated.
"""
...
from ntfy_alerts import alert_quarantine, alert_reauth
...
class TrustManager:
    def __init__(self, node_ids, send_live_alerts=True):
        self.scores = {node: 100 for node in node_ids}
        self.quarantined = set()
        self.clean_streak = {node: 0 for node in node_ids}
        self.log = []
        self.send_live_alerts = send_live_alerts
        self.alerts_sent = 0

    def _maybe_alert(self, alert_fn, *args):
        if not self.send_live_alerts:
            return
        if MAX_LIVE_ALERTS is not None and self.alerts_sent >= MAX_LIVE_ALERTS:
            return
        alert_fn(*args)
        self.alerts_sent += 1
    ...
    def record_result(self, node_id: str, sig_valid: bool, is_anomaly: bool, packet_idx: int):
        ...
        if not was_quarantined and score < TRUST_THRESHOLD:
            ...
            print(f"[ALERT] packet {packet_idx}: {node_id} -> QUARANTINED (score: {score})")
            self._maybe_alert(alert_quarantine, node_id, score, packet_idx)
            ...
            if is_clean and self.clean_streak[node_id] >= CLEAN_STREAK_TO_REAUTH:
                ...
                print(f"[ALERT] packet {packet_idx}: {node_id} -> RE-AUTHENTICATED (score: {score})")
                self._maybe_alert(alert_reauth, node_id, score, packet_idx)
...
def simulate_constellation(csv_path="../data/telemetry_with_predictions_v2.csv",
                            seed=99, send_live_alerts=True):
    rng = random.Random(seed)
    df = pd.read_csv(csv_path)

    node_ids = list(NODE_SECRETS.keys())
    trust_mgr = TrustManager(node_ids, send_live_alerts=send_live_alerts)
    results = []
    ...
    print(f"\n(Live alerts sent this run: {trust_mgr.alerts_sent})")
    print("\n=== Per-Node Summary ===")
...
if __name__ == "__main__":
    simulate_constellation(send_live_alerts=True)
